...
export interface WorkerItem {
  ...
  transferred_to_vault?: number | boolean
}
...
export const workerAdminApi = {
  ...
  zeroAllBalances: () => postAdmin<{ success: boolean }>('/api/admin/zero-balances', {}),
  ...
  broadcastChannel: (data?: { customMessage?: string; channel?: string }) => postAdmin<{ success: boolean; telegramResult: any }>('/api/admin/broadcast-channel', data || {}),
  sendGlobalBroadcast: (message: string) => postAdmin<{ success: boolean; count?: number; total?: number }>('/api/admin/broadcast-global', { message }),
  getArmies: () => fetchAdmin<any[]>('/api/admin/armies'),
}
